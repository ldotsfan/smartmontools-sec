...
"        security-setpass,[password], security-disable,[password], security-erase,[password], security-unlock,[password]\n"
...
static std::string getvalidarglist(int opt)
{
  ...
  case opt_set:
    return "aam,[N|off], apm,[N|off], dsn,[on|off], lookahead,[on|off], security-freeze, "
           "security-setpass, security-erase, security-disable, security-unlock, security-eeprom-setpass, security-eeprom-disable, security-eeprom-unlock, "
    ...
    case opt_set: // --set
      {
        ...
        int n1 = -1, n2 = -1, n3 = -1, len = strlen(optarg);
        if (!get && !strncmp(optarg,"security-eeprom-setpass,",strlen("security-eeprom-setpass,"))) {
				...
				else if (!get && !strncmp(optarg,"security-eeprom-disable,",strlen("security-eeprom-disable,"))) {
		    		memset(ataopts.set_security_password,0,33);
				...
				else if (!get && !strncmp(optarg,"security-erase,",strlen("security-erase,"))) {
				    memset(ataopts.set_security_password,0,33);
				...
				else if (!get && !strncmp(optarg,"security-eeprom-unlock,",strlen("security-eeprom-unlock,"))) {
				    memset(ataopts.set_security_password,0,33);
				...
				else if (!get && !strncmp(optarg,"security-setpass,",strlen("security-setpass,"))) {
				    memset(ataopts.set_security_password,0,33);
  			...
  			else if(!get && !strncmp(optarg,"security-disable,",strlen("security-disable,"))) {
			...
			else if(!get && !strncmp(optarg,"security-unlock,",strlen("security-unlock,"))) {
	    			memset(ataopts.set_security_password,0,32);
